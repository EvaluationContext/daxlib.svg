...
=== "Syntax"
    ...
    | maxTicks | <span class="type-label int64">INT64</span> | :material-close: | `#!dax 5` | Optional: Desired maximum number of ticks. Minimum is 2. Defaults to 5 |
    | includeZero | <span class="type-label boolean">BOOLEAN</span> | :material-close: | `#!dax FALSE()` | Optional: Whether to include zero in the range when data is fully positive. Defaults to FALSE. |
    			...
    			VAR _NiceInterval =
    				IF(
    					...
    					IF( 
                            _DataMax = 0, 
                            1, 
                            DaxLib.SVG.Scale.NiceNum( ABS( _DataMax ) * 0.2 ) 
                        ),
    					DaxLib.SVG.Scale.NiceNum( _Range / ( _MaxTicks - 1 ) )
    				)
